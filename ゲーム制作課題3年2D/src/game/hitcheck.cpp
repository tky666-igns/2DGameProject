...
void Hit::HitCheckPlayerToStage()
{
	...
	for (int y = 0; y < STAGE_Y; y++)
	{
		for (int x = 0; x < STAGE_X; x++)
		{
			...
			if (m_stageData[y][x].m_state != STAGEID_BLOCK)
			{
				continue;
			}
			//プレイヤーとマップを矩形同士の当たり判定で調べる
			bool isHit = ChenkHitSquareToSquare(m_pos,
				PLAYER_SIZE_X, PLAYER_SIZE_Y,
				m_stageData[y][x].m_pos,
			...
			int posX = (int)m_pos.x;
			int posY = (int)m_pos.y;
			...
			//めり込んだ距離を計算する
			//プレイヤーと物体の距離を計算する
			float lenX = m_pos.x - m_stageData[y][x].m_pos.x;
			float lenY = m_pos.y - m_stageData[y][x].m_pos.y;
			...
			if (lenX < lenY)
			{
				...
				if (m_stageData[y][x + posX].m_state != STAGEID_BLOCK)
				{
				...
				else if (m_stageData[y + posY][x].m_state != STAGEID_BLOCK)
				{
			...
			{
				//押し戻す方向にブロックがなければ横に押し戻す
				if (m_stageData[y + posY][x].m_state != STAGEID_BLOCK)
				{
					//posXをかけることにより左右の+-が正しく設定される
					lenY *= posY;
					//想定の方向にめり込んだ分だけ押し戻す
				...
				//横がダメだったときは第二候補の上下で押し戻す
				else if (m_stageData[y][x + posX].m_state != STAGEID_BLOCK)
				{
					//posYをかけることにより上下の+-が正しく設定される
					lenX *= posX;
					//想定の方向にめり込んだ分だけ押し戻す
